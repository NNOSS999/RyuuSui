	...
	$(function () {
		...
		function initOwlCarousel(c) {
			...
			for (var j = 0; j < values.length; j++) {
				responsive[values[j]] = {};
				for (var k = j; k >= -1; k--) {
					if (!responsive[values[j]]["items"] && c.attr("data" + aliases[k] + "items")) {
						responsive[values[j]]["items"] = k < 0 ? 1 : parseInt(c.attr("data" + aliases[k] + "items"), 10);
					}
					if (!responsive[values[j]]["stagePadding"] && responsive[values[j]]["stagePadding"] !== 0 && c.attr("data" + aliases[k] + "stage-padding")) {
						responsive[values[j]]["stagePadding"] = k < 0 ? 0 : parseInt(c.attr("data" + aliases[k] + "stage-padding"), 10);
					}
					if (!responsive[values[j]]["margin"] && responsive[values[j]]["margin"] !== 0 && c.attr("data" + aliases[k] + "margin")) {
						responsive[values[j]]["margin"] = k < 0 ? 30 : parseInt(c.attr("data" + aliases[k] + "margin"), 10);
					}
				}
			}
		...
		/**
		 * Bootstrap cards
		 * @description Activate Bootstrap cards
		 */
		if (plugins.bootstrapCards.length && isNoviBuilder) {
			for (let i = 0; i < plugins.bootstrapCards.length; i++) {
				let bootstrapCard = plugins.bootstrapCards[i];

				let cardHeads = bootstrapCard.querySelectorAll('.card-heading a');
